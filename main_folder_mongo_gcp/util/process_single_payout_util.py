...
async def process_single_payout(
    ...
    try:
        ...
        if not receiver_account_exists:
            ...
            logging.info(f"Receiver needs ATA creation. Fee: {ATA_CREATION_FEE_SOL} SOL = {ata_fee_in_usdc:.6f} USDC")
            ...
            logging.info(f"Adjusted USDC amount after fee deduction: {adjusted_amount_usdc} (original: {amount_usdc})")
        ...
        logging.info(f"Sent USDC transaction {tx_signature} for {amount_usdc} USDC{fee_info}")
