...
async def process_single_payout(
    ...
    try:
        logger.info(f"Processing USDC payment of {amount_usdc} to {wallet_address} for user {user_id}")
        ...
        if not receiver_account_exists:
            ...
            if adjusted_amount_usdc <= 0:
                return False, "", f"After ATA creation fee deduction ({ata_fee_in_usdc:.6f} USDC), the remaining amount would be zero or negative."
            
            logger.info(f"Adjusted USDC amount after fee deduction: {adjusted_amount_usdc} (original: {amount_usdc})")
            amount_usdc = adjusted_amount_usdc
        ...
        if not receiver_account_exists:
            logger.info(f"Creating token account for recipient {wallet_address}")
            create_ata_accounts = [
        ...
        # Add info about fee deduction
        fee_info = ""
        if not receiver_account_exists:
            fee_info = f" (ATA creation fee of {ATA_CREATION_FEE_SOL} SOL = {ata_fee_in_usdc:.6f} USDC was deducted from original amount of {original_amount_usdc})"
        
        logger.info(f"Sent USDC transaction {tx_signature} for {amount_usdc} USDC{fee_info}")
            ...
            try:
                ...
                if status:
                    ...
                    if is_finalized:
                        ...
                        if fee_info:
                            success_message += fee_info
                        return True, tx_signature, success_message
            except Exception as e:
        ...
        logger.error(f"Error processing USDC payment for {user_id}: {str(e)}")
