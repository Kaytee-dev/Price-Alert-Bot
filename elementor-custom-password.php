...
function custom_password_redirect() {
    ...
    error_log("📌 [Password Redirect] Cookie Set: " . json_encode($referrer_data));

    // 🔁 Redirect to custom password form page
    error_log("🔁 Redirecting to: " . site_url('/' . CUSTOM_PASSWORD_PAGE_SLUG));
        ...
        const getReferrerData = function () {
            ...
            try {
                ...
                if (lsData && lsData.url) {
                    ...
                    referrerData = lsData;
                } else if (cookieData && cookieData.url) {
                    console.log("📦 Using referrer data from cookie");
                    referrerData = cookieData;
                } else {
                    throw new Error("No usable data");
                }
            } catch (e) {
                console.warn("⚠️ Referrer data fallback triggered:", e);
                referrerData = {
                    id: 0,
                ...
                fetch("<?php echo esc_url(site_url('/wp-login.php?action=postpass')); ?>", {
                    ...
                    setTimeout(() => {
                        ...
                        if (post_password_required($post)) {
                            error_log("🔐 Tier 2 is STILL password protected!");
